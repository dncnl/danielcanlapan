...
function Certifications() {
  ...
  return (
    <section id="certifications" style={{ padding: 'var(--section-y) var(--page-pad-x)', background: 'linear-gradient(to bottom, var(--surface) 0, var(--bg) var(--space-10))' }}>
          ...
          {slides.map((slide, i) => (
            <Reveal key={slide.title} delay={60 * i}>
              <Card variant="hairline" className="hover-card" style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 'var(--space-3)', padding: 'var(--space-5)' }}>
                {slide.badge ? (
                  <img src={slide.badge} alt={`${slide.title} badge`} width={240} height={240} className="badge-float"
                    style={{ width: 240, height: 240, objectFit: 'contain', animationDelay: (i * 280) + 'ms' }} />
                ) : (
